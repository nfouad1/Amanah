...
export default function ContributeContent() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (campaignFromUrl && activeCampaigns.some((c: any) => c.id === campaignFromUrl)) {
      ...
      <header className="bg-white shadow-sm">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="text-sm text-gray-600">
            {formData.campaignId ? `${t('currency')}: ${campaignCurrency}` : t('selectCampaign')}
          </div>
        </div>
      </header>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {formData.campaignId && (
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                {(() => {
                  const sc = campaigns.find(c => c.id === formData.campaignId);
                  if (!sc) return null;
                  const pct = (sc.current / sc.target) * 100;
                  return (
                    <>
                      <p className="text-sm text-gray-600 mb-2">{sc.description}</p>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-600">{sc.contributors} {t('contributors')}</span>
                        <span className="text-primary-600 font-medium">{pct.toFixed(0)}% funded</span>
                      </div>
                    </>
                  );
                })()}
            ...
            <div className="border-t pt-6">
              ...
              {formData.isRecurring && (
                <div className={`${isRTL ? 'mr-8' : 'ml-8'} mb-4`}>
                  ...
                  <p className="text-sm text-gray-500 mt-2">{t('canCancelAnytime')}</p>
                </div>
              )}
              ...
              <p className="text-sm text-gray-500 mt-2 ml-8">{t('nameWillBeHidden')}</p>
            </div>

            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
              <h3 className="font-semibold text-amber-900 mb-2">{t('privacyPayment')}</h3>
              <p className="text-sm text-amber-800 mb-3">{t('manualTracking')}</p>
              <p className="text-sm text-amber-800">{t('privateContribDesc')}</p>
            </div>
